...
The two are checked differently on purpose. vleapp.py guards its startup with
'if __name__', so it can be imported outright, which catches import-time errors
as well as syntax. vleappGUI.py builds its window at module level, so importing
...
CLI_ENTRY_POINT = REPO_ROOT / 'vleapp.py'
GUI_ENTRY_POINT = REPO_ROOT / 'vleappGUI.py'


class TestEntryPoints(unittest.TestCase):

    def test_cli_entry_point_imports(self):
        """vleapp.py must import, and expose main()."""
        ...
        self.assertTrue(hasattr(module, 'main'), 'vleapp.py no longer exposes main()')

    def test_gui_entry_point_compiles(self):
        """vleappGUI.py must at least compile; it cannot be imported without a display."""
        ...
        try:
            ...
            self.fail(f'vleappGUI.py does not compile on this Python: '
