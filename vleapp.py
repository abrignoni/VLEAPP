...
def main():
    ...
    parser.add_argument('-t', choices=['fs', 'tar', 'zip', 'gz', 'file', 'raw', 'iva'], required=False, action="store",
                        help=("Specify the input type. "
                              ...
                              "'raw' for a raw disk image whose QNX6, ETFS, EFS, ext, FAT32 or exFAT volumes and QNX IFS boot images are read without mounting, "
    ...
    if args.create_profile_casedata:
        if os.path.isdir(args.create_profile_casedata):
            create_choice = ''
            print('-' * 55)
            print('Welcome to VLEAPP Profile or Case Data file creation\n')
            instructions = 'You can type:\n'
