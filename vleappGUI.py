...
pickModules()
GuiWindow.progress_bar_total = len(vleapp.tosearch)


layout = [  [sg.Text('Vehicle Logs, Events, And Properties Parser', font=("Helvetica", 22))],
            ...
            [sg.Frame(layout=[
                ...
                title='Select a file (tar/zip/gz) or directory of the target vehicle system extraction for parsing:')],
            [sg.Frame(layout=[
...
while True:
    ...
    if event == 'Process':
        ...
        if is_valid:
            ...
            wrap_text = True
            crunch_successful = vleapp.crunch_artifacts(search_list, extracttype, input_path, out_params, len(vleapp.tosearch)/s_items, wrap_text)
